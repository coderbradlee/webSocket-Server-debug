...
string uploads_init(size_t userid, string filename, int size, bool bin = false)
{
	std::unique_lock<std::mutex> locker(conLock_);
...
string write_data(size_t userid, string data)
{
	std::unique_lock<std::mutex> locker(conLock_);
...
void finish_upload(size_t userid)
{
	std::unique_lock<std::mutex> locker(conLock_);
	auto iter = uploads.find(userid);
	if (iter != uploads.end())
...
void server_send_data(WsServer& server, shared_ptr<WsServer::Connection> connection, string retString, string action)
{
	try
	{
...
void server_send(WsServer& server, shared_ptr<WsServer::Connection> connection, string retString, string action)
{
	...
	server.initsink->flush();
	//************************************************
	server_send_data(server, connection, retString, action);

}

string rename_file(string data_oldFile, string data_newFile)
{
	std::unique_lock<std::mutex> locker(conLock_);
	//uploadsinit((size_t)connection.get(),filename,size,true);
	string retString;
...
bool directory_is_empty(fs::path p)
{
...
bool delete_file(string file,bool delete_empty)
{
	std::unique_lock<std::mutex> locker(conLock_);
...
void listfile(ptree& listfiletree, string path)
{
	std::unique_lock<std::mutex> locker(directory_is_empty_lock);
	//cout << "directory_is_empty" << endl;
	//ptree filetree;
...
void serverResourceUpload(WsServer& server, string url)
{
	auto& websocket = server.endpoint[url];
	websocket.onmessage = [&server](shared_ptr<WsServer::Connection> connection, shared_ptr<WsServer::Message> message) {
		ptime now = second_clock::local_time();
		string now_str = to_iso_extended_string(now.date()) + " " + to_simple_string(now.time_of_day());
		try
		{
			...
			if (clientmessage.compare(0, 9, "#PREPARE#") == 0)
			{
				...
				if (overwrite)
				{
					retString = uploads_init((size_t)connection.get(), file, size);
				}
				else
				{
				...
				server_send(server, connection, retString, "upload");
			}
			else
			{
				string output_str;
				...
				if (x)
				{
					retString = write_data((size_t)connection.get(), output_str);
	...
	websocket.onopen = [&server](shared_ptr<WsServer::Connection> connection) {
	...
	websocket.onclose = [&server](shared_ptr<WsServer::Connection> connection, int status, const string& reason) {
		finish_upload((size_t)connection.get());
	...
	websocket.onerror = [&server](shared_ptr<WsServer::Connection> connection, const boost::system::error_code& ec) {
...
void serverResourceRename(WsServer& server, string url)
{
	...
	websocket.onmessage = [&server](shared_ptr<WsServer::Connection> connection, shared_ptr<WsServer::Message> message) {
		try
		{
			...
			if (action.compare("RENAME_FILE") == 0)
			{
				...
				server.initsink->flush();
				//************************************************			
				retString = rename_file(data_oldFile, data_newFile);
				server_send(server, connection, retString, "rename");
			}
			else
			{
				ptime now = second_clock::local_time();
...
void serverResourceDelete(WsServer& server, string url)
{
		...
		try
		{
			...
			if (action.compare("DELETE_FILES") == 0)
			{
				...
				for (ptree::iterator it = pdata_files.begin(); it != pdata_files.end(); ++it)
				{
					...
					if (delete_file(nfspath + file, pdata_deleteDir))
						count++;
				}
				...
				server_send(server, connection, retString, "delete");
...
void serverResourceList(WsServer& server, string url)
{
		...
		try
		{
			...
			if (action.compare("LIST_FILES") == 0)
			{
				...
				for (ptree::iterator it = pdatainclude.begin(); it != pdatainclude.end(); ++it)
				{
					...
					listfile(listfiletree, nfspath+"/"+storagePath+"/"+type);
				...
				server_send(server, connection, retString, "list");
